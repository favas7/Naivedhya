...
class _ManagerDetailScreenState extends State<ManagerDetailScreen> {
  ...
  String? _error;
  Map<String, bool> _uploadingImages = {};
  ...
  Future<void> _loadManagers() async {
    ...
    try {
      final managers = await _supabaseService.getManagers(widget.hotel.id!);
      if (mounted) {
        setState(() {
          _managers = managers;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.hotel.name} - Managers'),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Add your first manager for ${widget.hotel.name}',
              style: TextStyle(
                fontSize: 14,
